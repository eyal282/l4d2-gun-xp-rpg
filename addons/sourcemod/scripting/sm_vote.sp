...
// @return:
public int StartCustomVote(Handle plugin, int numParams)
{
	int  client, Percents;
...
int BeginVote(int client, char[] Arg1, char[] Arg2, char[] Subject, Handle Call)
{
	...
	if (percents > 100 || percents <= 0 || !IsStringNumber(Arg2))
	{
		ReplyToCommand(max(client, 0), "[SM] Error: The percents must be a value between 1 and 100");
		return 1;
	}
	else if (CoolDown)
	{
	...
	{
		ReplyToCommand(max(client, 0), "[SM] Error: Valid teams: @all / @survivors / @infected");
		return 3;
	}
...
void StartVGUIVote(int client, int Team, char[] Issue)
{
	...
	GetClientName(max(client, 0), Name, sizeof(Name));
...
stock int max(int num1, int num2)
{
	if(num1 > num2)
	{
		return num1;
